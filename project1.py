...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    Returns:
        ...
        parameters.
    """
    hinge_loss = max(0, 1 - (label * ((theta @ feature_vector) + theta_0)))
    return hinge_loss
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range(len(feature_matrix)):
        cumm_hinge_loss += hinge_loss_single(feature_matrix[i], labels[i], theta, theta_0)
    return cumm_hinge_loss/len(feature_matrix)
    raise NotImplementedError
